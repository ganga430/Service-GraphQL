...
export const configs: GraphQLConfiguration = {
  ci: {
    aws: {
      'us-west-2': {
        ...
        awsApplicationLoadBalancer: 'alb-preprod-api.mgmresorts.com/green'
      }
    },
    apigee: {
      keyValueMaps: {
  ...
  all: {
    environmentVariables: {
      NODE_ENV: GQLEnvironment.PreProdGreen,
      SSM_PREFIX: '/ServicesGQL/preprod-green',
      ENABLE_DEBUG: 'false',
      SSM_PARAMETERS_TO_FETCH: ssmParametersToFetch['preprod-green'],
      AS_BASE_URL: 'https://mgm-nonprod-preprod.apigee.net/guestservices/as/api/'
    }
  },
  gateway: {
    environmentVariables: {
      APOLLO_GRAPH_VARIANT: GQLEnvironment.PreProdGreen
    },
    local: {
      playgroundUrl: `https://dev-graphqlgw.mgmresorts.com/${GQLEnvironment.PreProdGreen}/graphql`
    }
  },
  customer: {
    environmentVariables: {
  ...
  'cs-content': {
    environmentVariables: {
      ...
      CS_PUBLISHING_ENVIRONMENT: 'preprod-green',
      CS_BRANCH_NAME: 'preprod_green',
      CS_BRANCH_ALIAS: 'preprod_green'
    },
    local: {
